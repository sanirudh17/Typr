...
2. Under **Assets**, download **`Typr_0.1.5_x64-setup.exe`** *(the `.msi` is an alternative if your workplace requires MSI)*.
...
Installers land in:
- `src-tauri/target/release/bundle/nsis/Typr_0.1.5_x64-setup.exe` (~278 MB)
- `src-tauri/target/release/bundle/msi/Typr_0.1.5_x64_en-US.msi` (~436 MB)
...
```json
{
  ...
  "platforms": {
    "windows-x86_64": {
      ...
      "url": "https://github.com/sanirudh17/Typr/releases/download/v0.1.5/Typr_0.1.5_x64-setup.exe"
    }
  }
}
```
